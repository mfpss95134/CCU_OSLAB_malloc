...
typedef struct malloc_pool {
    ...
    struct block *next; //暫時用不到
} pool_t;


size_t POOL_SIZE = 1024*1024; // 1 GiB
pool_t MAIN_POOL;


MAIN_POOL.ptr = mmap(NULL, 10*POOL_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0);
if (MAIN_POOL.ptr == NULL)
{
...
__libc_malloc (size_t bytes)
{
  ...
  void *caller_ip = __builtin_return_address(0);  //獲得呼叫這個 malloc() 的呼叫者的 ip
